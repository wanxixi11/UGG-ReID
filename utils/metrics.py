...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=10):
    ...
    # # indices1 = torch.argsort(torch.tensor(distmat), dim=1, descending=True)
    # pred_labels = g_pids[torch.tensor(indices).cpu()]  # q * k
    # matches1 = torch.tensor(pred_labels).eq(torch.tensor(q_pids).view(-1, 1))
    # tmp_cmc1 = matches1.cumsum(1)  # q * k
    # inp = [tmp_cmc1[i][match_row.nonzero()[-1]] / (match_row.nonzero()[-1] + 1.) for i, match_row in enumerate(matches1)]
...
class R1_mAP_eval():
    ...
    def reset(self):
        ...
        self.img_paths = []
        self.img_prefixes = {
            'RGB': '/data/wanxixi/CLIP/Demo/data/RGBNT201/test/RGB/',
            'NIR': '/data/wanxixi/CLIP/Demo/data/RGBNT201/test/NI/',
            'TIR': '/data/wanxixi/CLIP/Demo/data/RGBNT201/test/TI/'
        }
    ...
    def visualize_ranked_results(self, distmat, topk=5, save_dir='vis_results'):
        """
        ...
        num2vis = 600  # Number of queries to visualize
    ...
    #     """
    #     num_results = len(gallery_imgs[0])  # Number of top-ranked results
    #     fig, axs = plt.subplots(3, num_results + 1, figsize=(14, 8), 
    #                            gridspec_kw={'wspace': 0.05, 'hspace': 0.05})  # 设置行列间距为0

    #     # Display the query images
    #     modalities = ['RGB', 'NIR', 'TIR']
    #     for j, (img, modality) in enumerate(zip(query_imgs, modalities)):
    #         axs[j, 0].imshow(img)
    #         axs[j, 0].axis('off')  # 关闭坐标轴
    #         # axs[j, 0].set_title(f"Query {modality}", fontsize=21, pad=10)  # 仅保留标题

    #     # Display the top-ranked results
    #     for i, (imgs, pid) in enumerate(zip(zip(*gallery_imgs), gallery_pids)):
    #         for j, (img, modality) in enumerate(zip(imgs, modalities)):
    #             axs[j, i + 1].imshow(img)
    #             axs[j, i + 1].axis('off')
    ...
    #             axs[j, i + 1].add_patch(rect)
    #             # axs[j, i + 1].set_title(f"Rank {i + 1}", fontsize=22, pad=10)  # 仅保留标题

    #     plt.subplots_adjust(left=0.05, right=0.95, top=0.9, bottom=0.1)  # 调整整体边距
    #     if save_path:
    #         plt.savefig(save_path, bbox_inches='tight', pad_inches=0.1)  # 保存时无额外边距
    #     plt.close()
    def plot_images(self, query_imgs, gallery_imgs, gallery_pids, query_pid, save_path=None):
        """
        Helper function to plot the query images and the gallery images.
        ...
        """
        num_results = len(gallery_imgs[0])  # Number of top-ranked results
        fig, axs = plt.subplots(1, num_results + 1, figsize=(20, 6), 
                               gridspec_kw={'wspace': 0.05})  # 改为1行多列布局

        # Display the query image (水平拼接三个模态)
        merged_query = np.concatenate(query_imgs, axis=1)  # 改为水平拼接
        axs[0].imshow(merged_query)
        axs[0].axis('off')

        # Display the top-ranked results (水平拼接三个模态)
        for i, (imgs, pid) in enumerate(zip(zip(*gallery_imgs), gallery_pids)):
            # 水平拼接三个模态的图像
            merged_img = np.concatenate(imgs, axis=1)  # 改为水平拼接
            axs[i+1].imshow(merged_img)
            axs[i+1].axis('off')

            # Add a green or red rectangle around the image
            color = 'green' if pid == query_pid else 'red'
            rect = patches.Rectangle((0, 0), merged_img.shape[1], merged_img.shape[0],
                                   linewidth=10, edgecolor=color, facecolor='none')
    ...
    def compute(self):  # called after each epoch
        ...
        # self.visualize_ranked_results(distmat, topk=10, save_dir='rankList/ug2moe')
        # self.visualize_ranked_results(distmat, topk=10, save_dir='rankList/base')
